...
public class Connexion_EP_steps {
	...
	@And("^L'utilisateur voit le menu 'favoris'$")
	public void verifier_menu_favoris() {
		WebDriverWait wait= new WebDriverWait(driver,20);	
		WebElement titreFavoris=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[contains(text(),'Favoris')]")));
		Assert.assertEquals(titreFavoris.getText(),"Favoris");
	}
